export interface AttachmentRejectedData {
...
export function generateAttachmentRejectedEmail(
...
<head>
  ...
  <title>Archivo Rechazado - PeopleFlow</title>
</head>
          ...
          <tr>
            <td style="padding: 32px 40px;">
              ...
              <p style="margin: 0 0 24px 0; font-size: 16px; line-height: 1.6; color: #3f3f46;">
                El archivo <strong style="color: #9333ea;">${fileName}</strong> de tu vacante <strong style="color: #9333ea;">${vacancyPosition}</strong> (${clientName}) fue rechazado.
              </p>
                ...
                <tr>
                  <td style="background-color: #f3e8ff; border-left: 4px solid #9333ea; border-radius: 4px; padding: 16px;">
                    <p style="margin: 0; font-size: 14px; color: #6b21a8; line-height: 1.5;">
                      Por favor sube un nuevo archivo para continuar con el proceso de la vacante.
                    </p>
                  </td>
                </tr>
              </table>
          ...
          <tr>
            <td align="center" style="padding: 24px 20px;">
              <p style="margin: 0; font-size: 12px; color: #71717a;">
                Este correo fue enviado porque un archivo de una de tus vacantes fue rechazado en PeopleFlow.
              </p>
            </td>
          </tr>
        </table>
...
export function generateAttachmentRejectedPlainText(
  data: AttachmentRejectedData
): string {
  const { recruiterName, vacancyPosition, clientName, fileName, rejectionReason, appUrl, vacancyId } = data;
  const vacancyUrl = `${appUrl}/reclutamiento/vacantes?vacancyId=${vacancyId}`;
...
El archivo "${fileName}" de tu vacante ${vacancyPosition} (${clientName}) fue rechazado.
